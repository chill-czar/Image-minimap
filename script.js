...
maxTranslate = dimension.containerSize - dimension.indicatorSize;

function getItemInIndicator() {
  itemImages.forEach((img) => (img.style.opacity = 1));

  const indicatorStart = -currentTranslate;
  const indicatorEnd = indicatorStart + dimension.indicatorSize;

  let maxOverlap = 0;
  let selectedIndex = 0;

  itemElements.forEach((item, index) => {
    const itemStart = index * dimension.itemSize;
    const itemEnd = itemStart + dimension.itemSize;
...
function animate() {
  const lerpFactor = isClickMove ? 0.05 : 0.75;
...
itemElements.forEach((item, index) => {
  item.addEventListener("click", () => {
    isClickMove = true;
    targetTranslate =
      -index * dimension.itemSize +
      (dimension.indicatorSize - dimension.itemSize) / 2;
...
window.addEventListener("resize", () => {
  ...
  const newMaxTranslate = dimension.containerSize - dimension.indicatorSize;
